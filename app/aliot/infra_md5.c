...
static FUNC(void) utils_md5_zeroize(void *v, size_t n)
{
...
FUNC(void) utils_md5_init(iot_md5_context *ctx)
{
    memset(ctx, 0, sizeof(iot_md5_context));
}

FUNC(void) utils_md5_free(iot_md5_context *ctx)
{
    if (ctx == NULL) {
        return;
    }
...
FUNC(void) utils_md5_clone(iot_md5_context *dst,
 ...
 * MD5 context setup
 */
FUNC(void) utils_md5_starts(iot_md5_context *ctx)
{
    ctx->total[0] = 0;
    ctx->total[1] = 0;
...
FUNC(void) utils_md5_process(iot_md5_context *ctx, const unsigned char data[64])
{
 ...
 * MD5 process buffer
 */
FUNC(void) utils_md5_update(iot_md5_context *ctx, const unsigned char *input, uint32_t ilen)
{
    uint32_t fill;
    uint32_t left;
 ...
 * MD5 final digest
 */
FUNC(void) utils_md5_finish(iot_md5_context *ctx, unsigned char output[16])
{
    uint32_t last, padn;
    uint32_t high, low;
 ...
 */
FUNC(void) utils_md5(const unsigned char *input, uint32_t ilen, unsigned char output[16])
{
    iot_md5_context ctx;
...
FUNC(void) utils_hmac_md5_hex(const char *msg, int msg_len, const char *key, int key_len, char output[MD5_DIGEST_SIZE])
{
    ...
    utils_md5_finish(&context, output);                               /* finish up 1st pass */
    ...
    utils_md5_update(&context, output, MD5_DIGEST_SIZE);      /* then results of 1st hash */
    utils_md5_finish(&context, output);                       /* finish up 2nd pass */
}

FUNC(void) utils_hmac_md5(const char *msg, int msg_len, const char *key, int key_len, char output[MD5_DIGEST_SIZE*2])
{
    int i;
    char out[MD5_DIGEST_SIZE];
    utils_hmac_md5_hex(msg, msg_len, key, key_len, out);
