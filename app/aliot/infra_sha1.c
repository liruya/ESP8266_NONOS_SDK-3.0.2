...
static FUNC(void) utils_sha1_zeroize( void *v, uint32_t n ) {
...
FUNC(void) utils_sha1_init(iot_sha1_context *ctx)
{
    memset(ctx, 0, sizeof(iot_sha1_context));
}

FUNC(void) utils_sha1_free(iot_sha1_context *ctx)
{
    if (ctx == NULL) {
        return;
    }
...
FUNC(void) utils_sha1_clone(iot_sha1_context *dst,
 ...
 * SHA-1 context setup
 */
FUNC(void) utils_sha1_starts(iot_sha1_context *ctx)
{
    ctx->total[0] = 0;
    ctx->total[1] = 0;
...
FUNC(void) utils_sha1_process(iot_sha1_context *ctx, const unsigned char data[64])
{
 ...
 */
FUNC(void) utils_sha1_update(iot_sha1_context *ctx, const unsigned char *input, uint32_t ilen)
{
    uint32_t fill;
    uint32_t left;
 ...
 * SHA-1 final digest
 */
FUNC(void) utils_sha1_finish(iot_sha1_context *ctx, unsigned char output[20])
{
    uint32_t last, padn;
    uint32_t high, low;
 ...
 */
FUNC(void) utils_sha1(const unsigned char *input, uint32_t ilen, unsigned char output[20])
{
...
static FUNC(int8_t) utils_hb2hex(uint8_t hb)
{
    hb = hb & 0xF;
    return (int8_t)(hb < 10 ? '0' + hb : hb - 10 + 'A');
}

FUNC(void) utils_hmac_sha1_hex(const char *msg, int msg_len, const char *key, int key_len, char output[SHA1_DIGEST_SIZE])
{
    ...
    if ((NULL == msg) || (NULL == output) || (NULL == key)) {
        return;
    }
    ...
    utils_sha1_finish(&context, output);                               /* finish up 1st pass */
    ...
    utils_sha1_update(&context, output, SHA1_DIGEST_SIZE);     /* then results of 1st hash */
    utils_sha1_finish(&context, output);                       /* finish up 2nd pass */
}

FUNC(void) utils_hmac_sha1(const char *msg, int msg_len, const char *key, int key_len, char output[SHA1_DIGEST_SIZE*2])
{
    int i;
    char out[SHA1_DIGEST_SIZE];
    utils_hmac_sha1_hex(msg, msg_len, key, key_len, out);
