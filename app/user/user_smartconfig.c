...
LOCAL void ESPFUNC user_smartconfig_done(sc_status status, void *pdata) {
	switch (status) {
		case SC_STATUS_WAIT:
			DBG(TAG, "SC_STATUS_WAIT\n");
			break;
		case SC_STATUS_FIND_CHANNEL:
			DBG(TAG, "SC_STATUS_FIND_CHANNEL\n");
			break;
		case SC_STATUS_GETTING_SSID_PSWD:
			DBG(TAG, "SC_STATUS_GETTING_SSID_PSWD\n");
			sc_type *type = (sc_type *) pdata;
			if (*type == SC_TYPE_ESPTOUCH) {
				DBG(TAG, "SC_TYPE:SC_TYPE_ESPTOUCH\n");
			} else {
				DBG(TAG, "SC_TYPE:SC_TYPE_AIRKISS\n");
			}
			break;
		case SC_STATUS_LINK:
			DBG(TAG, "SC_STATUS_LINK\n");
		...
		case SC_STATUS_LINK_OVER:
			DBG(TAG, "SC_STATUS_LINK_OVER\n");
			if (pdata != NULL) {
				//SC_TYPE_ESPTOUCH
				uint8 phone_ip[4] = { 0 };
				os_memcpy(phone_ip, (uint8*) pdata, 4);
				DBG(TAG, "Phone ip: %d.%d.%d.%d\n", phone_ip[0], phone_ip[1],
...
LOCAL bool ESPFUNC user_smartconfig_start(task_t **task) {
	DBG(TAG, "smartconfig start... %d  *%d  **%d", task, *task, **task);
...
LOCAL bool ESPFUNC user_smartconfig_stop(task_t **task) {
	DBG(TAG, "smartconfig stop... %d  *%d  **%d", task, *task, **task);
	if (task == NULL || *task == NULL) {
		return false;
	}
	smartconfig_stop();
...
void ESPFUNC user_smartconfig_instance_start(const task_impl_t *impl, const uint32_t timeout) {
    if (sc_task != NULL) {
        ERR(TAG, "smartconfig start failed -> already started...");
        return;
    }
    sc_task = os_zalloc(sizeof(smartconfig_task_t));
    if (sc_task == NULL) {
        ERR(TAG, "smartconfig start failed -> malloc sc_task failed...");
        return;
    }
	DBG(TAG, "smartconfig create... %d", sc_task);
